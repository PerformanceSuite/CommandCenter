...
class DetectedTechnology(BaseModel):
    ...
    category: str = Field(..., description="Category (framework, database, build_tool, etc.)")
...
class CodeMetrics(BaseModel):
    ...
    architecture_pattern: Optional[str] = Field(None, description="Detected architecture pattern")
    ...
    total_classes: Optional[int] = Field(None, ge=0, description="Total number of classes detected")
...
class AnalysisStatistics(BaseModel):
    ...
    total_technologies: int = Field(..., ge=0, description="Total technologies detected")
    ...
    outdated_dependencies: int = Field(..., ge=0, description="Outdated dependencies count")
...
class GenerateTasksRequest(BaseModel):
    ...
    severity_threshold: Optional[str] = Field(
        None,
        description="Only generate tasks for gaps at or above this severity",
    )
    auto_assign: bool = Field(default=False, description="Automatically assign tasks to users")
