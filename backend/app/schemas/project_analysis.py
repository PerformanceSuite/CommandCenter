...
class Dependency(BaseModel):
    ...
    is_outdated: bool = Field(
        default=False, description="Whether version is outdated"
    )
...
class CodeMetrics(BaseModel):
    ...
    complexity_score: float = Field(
        ..., ge=0.0, description="Code complexity score"
    )
...
class GitHubAnalysisRequest(BaseModel):
    ...
    branch: Optional[str] = Field(
        None, description="Branch to analyze (default: main)"
    )
    use_cache: bool = Field(
        default=True, description="Use cached analysis if available"
    )
...
class ProjectAnalysisResult(BaseModel):
    ...
    code_metrics: CodeMetrics = Field(
        ..., description="Code structure metrics"
    )
    ...
    analysis_duration_ms: int = Field(
        ..., ge=0, description="Analysis duration in ms"
    )
...
class ProjectAnalysisResponse(BaseModel):
    ...
    analysis_version: str = Field(
        ..., description="Analysis version for cache"
    )
...
class AnalysisStatistics(BaseModel):
    ...
    total_analyses: int = Field(
        ..., ge=0, description="Total number of analyses"
    )
    unique_projects: int = Field(
        ..., ge=0, description="Unique projects analyzed"
    )
    total_dependencies: int = Field(
        ..., ge=0, description="Total dependencies tracked"
    )
    total_technologies: int = Field(
        ..., ge=0, description="Total technologies detected"
    )
    total_gaps: int = Field(
        ..., ge=0, description="Total research gaps identified"
    )
