...
class MetricsService:
    ...
    def update_rate_limit_metrics(
        resource_type: str, remaining: int, limit: int
    ):
        """
        ...
        Args:
            ...
            limit: Total limit
        """
        github_api_rate_limit_remaining.labels(
            resource_type=resource_type
        ).set(remaining)
        github_api_rate_limit_limit.labels(resource_type=resource_type).set(
            limit
        )
    ...
    def record_repository_sync(repository: str, status: str, duration: float):
        ...
        repository_sync_duration_seconds.labels(repository=repository).observe(
            duration
        )
