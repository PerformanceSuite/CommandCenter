...
def upgrade() -> None:
    ...
    op.add_column(
        "webhook_configs",
        sa.Column("delivery_mode", sa.String(20), nullable=False, server_default="async"),
    )
    op.add_column(
        "webhook_configs",
    ...
    op.add_column(
        "webhook_configs",
        sa.Column("max_delivery_time_seconds", sa.Integer(), nullable=False, server_default="3600"),
    )
