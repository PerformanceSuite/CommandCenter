...
async def test_technology_search_prevents_sql_injection(client, auth_headers_factory, user_a):
    ...
    response = await client.get(
        f"/api/v1/technologies?search={malicious_query}",
        headers=headers
    )
...
@pytest.mark.asyncio
async def test_repository_owner_filter_prevents_sql_injection(
    ...
    headers = auth_headers_factory(user_a)
    response = await client.get(
        f"/api/v1/repositories?owner={malicious_owner}",
        headers=headers
    )
...
@pytest.mark.asyncio
async def test_research_task_search_prevents_sql_injection(
    client, auth_headers_factory, user_a
):
    ...
    headers = auth_headers_factory(user_a)
    response = await client.get(
        f"/api/v1/research?q={malicious_search}",
        headers=headers
    )
