...
class TestHTTPTransport:
    ...
    async def test_health_endpoint(self, http_client):
        ...
        response = await http_client.get("/mcp/v1/health")
    ...
    async def test_info_endpoint(self, http_client):
        ...
        response = await http_client.get("/mcp/v1/info")
        assert response.status_code == 200
        data = response.json()
    ...
    @pytest.mark.asyncio
    async def test_rpc_endpoint_list_resources(self, http_client):
        """Test JSON-RPC request for listing resources."""
        rpc_request = {"jsonrpc": "2.0", "id": 1, "method": "resources/list", "params": {}}
        response = await http_client.post("/mcp/v1/rpc", json=rpc_request)
        assert response.status_code == 200
        data = response.json()
    ...
    @pytest.mark.asyncio
    async def test_rpc_endpoint_list_tools(self, http_client):
    ...
    @pytest.mark.asyncio
    async def test_rpc_endpoint_list_prompts(self, http_client):
    ...
    @pytest.mark.asyncio
    async def test_rpc_endpoint_invalid_json(self, http_client):
        """Test JSON-RPC endpoint with invalid JSON."""
        response = await http_client.post("/mcp/v1/rpc", content="invalid json")
        assert response.status_code >= 400

    @pytest.mark.asyncio
    async def test_rpc_endpoint_invalid_method(self, http_client):
    ...
    @pytest.mark.asyncio
    async def test_create_http_app_returns_fastapi(self, mcp_server):
    ...
    @pytest.mark.asyncio
    async def test_rpc_notification_no_response(self, http_client):
        """Test JSON-RPC notification (no id, no response expected)."""
        rpc_notification = {"jsonrpc": "2.0", "method": "notification/test", "params": {}}
        response = await http_client.post("/mcp/v1/rpc", json=rpc_notification)
