...
class LLMGateway:
    ...
    def __init__(self, registry: Optional[DynamicProviderRegistry] = None) -> None:
        ...
        Args:
            registry: Optional DynamicProviderRegistry for database-backed provider configs.
                     If None, uses static provider configurations.
        """
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        """
        # Get provider config from registry if available, otherwise use static config
        if self.registry:
